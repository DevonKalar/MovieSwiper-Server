...
describe("fetchMovieDetails", () => {
  ...
  it("returns movie data for a valid id", async () => {
    const movie = { id: 550, title: "Fight Club", overview: "...", release_date: "1999-10-15" };
    server.use(
...
describe("fetchPopularMovies", () => {
  ...
  it("returns the full response for the default page", async () => {
    const payload = { page: 1, results: [{ id: 1, title: "Popular" }], total_pages: 5, total_results: 100 };
    server.use(
      http.get(`${TMDB_BASE}/movie/popular`, () => HttpResponse.json(payload)),
    );
  ...
  it("passes the page param to the request", async () => {
    ...
    server.use(
      http.get(`${TMDB_BASE}/movie/popular`, ({ request }) => {
        ...
        return HttpResponse.json({ page: 3, results: [], total_pages: 5, total_results: 100 });
      }),
    );
...
describe("fetchMoviesByQuery", () => {
  ...
  it("returns movies matching the query", async () => {
    const payload = { page: 1, results: [{ id: 2, title: "Action Movie" }], total_pages: 1, total_results: 1 };
    server.use(
      http.get(`${TMDB_BASE}/discover/movie`, () => HttpResponse.json(payload)),
    );
...
describe("fetchMoviesByGenre", () => {
  ...
  it("returns movies for the given genre", async () => {
    const payload = { page: 1, results: [{ id: 3, title: "Horror Movie" }], total_pages: 1, total_results: 1 };
    server.use(
      http.get(`${TMDB_BASE}/discover/movie`, () => HttpResponse.json(payload)),
    );
  ...
  it("forwards genre id and page to the request", async () => {
    ...
    server.use(
      http.get(`${TMDB_BASE}/discover/movie`, ({ request }) => {
        ...
        return HttpResponse.json({ page: 2, results: [], total_pages: 3, total_results: 60 });
      }),
    );
...
describe("fetchGenres", () => {
  ...
  it("returns the genres array from the response", async () => {
    const genres = [{ id: 28, name: "Action" }, { id: 27, name: "Horror" }];
    server.use(
  ...
  it("returns an empty array when the API responds with an error", async () => {
    server.use(
      http.get(`${TMDB_BASE}/genre/movie/list`, () => new HttpResponse(null, { status: 500 })),
    );
