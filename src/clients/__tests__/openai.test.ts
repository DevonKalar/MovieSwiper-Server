...
describe("createResponse", () => {
  ...
  it("returns the API response for a basic input", async () => {
    const payload = { id: "resp_001", object: "response", status: "completed", output: "Hello!" };
    server.use(
      http.post(`${OPENAI_BASE}/responses`, () => HttpResponse.json(payload)),
    );
...
describe("retrieveResponse", () => {
  ...
  it("returns the response for a given id", async () => {
    const payload = { id: "resp_001", object: "response", status: "completed", output: "Hello!" };
    server.use(
      http.get(`${OPENAI_BASE}/responses/resp_001`, () => HttpResponse.json(payload)),
    );
